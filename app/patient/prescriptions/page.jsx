...
import { Pill, Calendar, Info, Loader2 } from 'lucide-react';
...
export default function Prescriptions() {
  ...
  useEffect(() => {
    (async () => {
      try { setRxList(await prescriptionService.getAll()); } catch { /* empty */ }
      setLoading(false);
    })();
  }, []);

  const tabs = [
    { id: 'active', label: 'Active', count: rxList.filter(p => p.status === 'active').length },
    { id: 'completed', label: 'Completed', count: rxList.filter(p => p.status === 'completed').length },
  ];
  const filtered = rxList.filter(p => p.status === tab);
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="grid gap-3">
        ...
        {!loading && filtered.length === 0 && <div className="text-center py-16 text-surface-400">No {tab} prescriptions.</div>}
      </div>
    </div>
  );
}
