...
import { Sun, Moon, Bell, Globe, Shield, Save, CheckCircle2 } from 'lucide-react';
    ...
    <div className="max-w-3xl mx-auto">
      ...
      <Card className="mb-6">
        <div className="p-6 border-b border-surface-100 dark:border-surface-800 flex items-center gap-4">
          <Avatar name={user?.name || 'Sarah Khan'} size="xl" />
          <div><h3 className="text-lg font-bold text-surface-900 dark:text-white">{user?.name || 'Sarah Khan'}</h3><p className="text-sm text-surface-400">{user?.email || 'sarah.khan@email.com'}</p><p className="text-xs text-surface-400 mt-0.5 capitalize">Patient Account</p></div>
        </div>
        <div className="p-6 grid sm:grid-cols-2 gap-4">
          <Input label="Full Name" defaultValue={user?.name || 'Sarah Khan'} />
          <Input label="Email" type="email" defaultValue={user?.email || 'sarah.khan@email.com'} />
          <Input label="Phone" defaultValue="+92-301-2345678" />
          <Input label="Date of Birth" type="date" defaultValue="1995-03-15" />
          <Input label="Blood Group" defaultValue="A+" />
          <Input label="Address" defaultValue="House 42, Street 7, Islamabad" />
        </div>
        <div className="px-6 pb-6 flex items-center gap-3"><Button onClick={handleSave}><Save size={16} /> Save Changes</Button>{saved && <span className="flex items-center gap-1 text-sm text-success-500"><CheckCircle2 size={14} /> Saved</span>}</div>
      </Card>
